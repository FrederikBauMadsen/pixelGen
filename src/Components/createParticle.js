...
export default function createParticle(pixel, columns, pixelSpace, effectName,stage){
  let array =[];
  let arrayPos = [];
...
//size should be 6,8,10,12 so on
  if(effectName=="stars"){
    ...
    for(var i = 0; i < random; i++){
      for(var pixels=0; pixels < (random*0.1); pixels+=0.001){
        let particle = SuperFormula(m,n1,n2,n3,(pixels*(Math.PI*2))/(random*0.1),i,i);
  ...
  let n3 = 1;
  for(var i = 0; i < random; i++){
    for(var pixels=0; pixels < (random*0.01); pixels+=0.001){
      let particle = SuperFormula(m,n1,n2,n3,(pixels*(Math.PI*2))/(random*0.01),i,i);
      let x = Math.round(particle.x);
      let y = Math.round(particle.y);
      let pos = parseInt(pixel+(x*pixelSpace)+(y*columns));
      let remainder = pos%4;
      ...
      if(y < 0 && Math.abs(x) < 9){
        if(arrayPos.includes(newPos)){
          ...
          if((y === -1 && Math.abs(x) > 2 )|| (y === -2 && Math.abs(x) > 3 ) || (y === -3 && Math.abs(x) > 4 ) || (y === -4 && Math.abs(x) > 5 ) || (y === -5 && Math.abs(x) > 6 ) || (y === -6 && Math.abs(x) > 7 ) || (y === -7 && Math.abs(x) > 8 ) || (y === -8 && Math.abs(x) > 9 ) ){
            continue;
          }else{
            //black/yellow/red
            array.push({
             pixel:newPos,
             r:0+(Math.abs(y)*(random*0.8863))+(Math.abs(x)*(random*0.8863)),
             g:0+(Math.abs(y)*(random*0.3451))+(Math.abs(x)*(random*0.3451)),
             b:0+(Math.abs(y)*(random*0.1333))+(Math.abs(x)*(random*0.1333))
           });
